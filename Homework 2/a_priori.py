...
def pruning(frequent_items, combinatory_factor):
    ...
    print(f"k = {combinatory_factor}\n free_keys = {list(frequent_items.keys())}\n  singletons = {singletons}")
...
def candidate_k_pairs(frequent_items, original_frequent_items, combinatory_factor, min_support):
    ...
    print(f"k = {combinatory_factor}  candidates = {candidates}  len = {len(candidates)}")
...
def main():
    start = time()
    k_tuple = 2  # Maximum size of association
    ...
    with open("T10I4D100K.dat", 'r') as file:
        for i, line in enumerate(T):
            n_transactions += 1
            transaction = [num for num in line.strip().split(' ')]
